...
import dlib, yt_dlp, requests, os, time, shutil,random, face_recognition, pickle, re
...
def download_youtube_video(youtube_url, output_path):
    ...
    # yt-dlp options
    ydl_opts = {
        'outtmpl': output_file,   # Save the file with the video title as the filename
        # 'format': 'bv*[height<=720]+ba/b[height<=720] / wv*+ba/w',
        'format': 'bv*[height<=640]+ba/b[height<=640] / wv*+ba/w',    # Download only the best video format
        # 'format': 'bestvideo', 
...
def app():
    ...
    font = '../templates/RACE1_Brannt_Plus_NCV.ttf'
    global song_name
    flag = True

    # try:

    if not os.path.exists("../thumbnails/frames1"):
        os.makedirs('../thumbnails/frames1')

    if not os.path.exists('../thumbnails/frames2'):
        os.makedirs('../thumbnails/frames2')

    unique_thumbnails_folder = "../thumbnails/frames3"
    if not os.path.exists('../thumbnails/frames3'):
        os.makedirs('../thumbnails/frames3')

    if not os.path.exists('../thumbnails/aiImages'):
        os.makedirs('../thumbnails/aiImages')
    ...
    if youtube_url:
        ...
        # shutil.copy('main_thumbnail.jpg', unique_thumbnails_folder)
    ...
    save_folder = "../thumbnails/saved_images"
    if not os.path.exists(save_folder):
        os.makedirs(save_folder)

    dump_thumbnails_folder = '../thumbnails/dump'
    ...
    if os.path.exists(unique_thumbnails_folder):
        ...
        if st.button("Shuffle Thumbnails"):
            # if os.path.exists('main_thumbnail.jpg'):
            #     os.remove('downloads/frames3/main_thumbnail.jpg')
            if len(os.listdir('../thumbnails/frames3'))==0:
                st.write("No more unique thumbnails to display...")
            else:
                for file_name in st.session_state.selected_thumbnails:
                    if file_name == 'main_thumbnail.jpg':
        ...
        if not st.session_state.selected_thumbnails:
            if thumbnails:
                ...
                if os.path.exists('../thumbnails/main_thumbnail.jpg'):
                ...
                if len(thumbnails) < thumb_count:
                    ...
                    if video_type == 'Song':
                        ...
                        mainActors = [f for f in os.listdir(unique_thumbnails_folder) if recognize_actors(f'../thumbnails/frames3/{f}', actors_list)]
        ...
        for idx, photo in enumerate(selected_thumbnails):
            if photo == 'main_thumbnail.jpg':
                img = Image.open(os.path.join(output_path, photo))
            else:
        ...
        if st.session_state.selected_thumbnails:
            ...
            if st.session_state.ai_edit_active:
                ...
                st.session_state.font = st.sidebar.radio('Font', options=['English', 'Hindi', 'Kannada'])
                ...
                if st.button("Submit"):
                    print("@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@")
                    print("FONT STYLE CHOOSEN", st.session_state.font)
                    print("@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@")
                    for i in images:
                        ...
                        out = f'../thumbnails/aiImages/aiImg_{cnt}.jpg'
                        if st.session_state.text: 
                            song_name = st.session_state.text

                        if st.session_state.font == 'English':
                            font = '../templates/RACE1_Brannt_Plus_NCV.ttf'
                            language = 'English'
                        elif st.session_state.font == 'Hindi':
                            font = '../templates/Mangal_Regular.ttf'
                            language = 'Hindi'
                        ai_image = process_image(inp, out, song_name, st.session_state.text_color,
                                        ...
                                        st.session_state.hdr, st.session_state.gray, st.session_state.color, st.session_state.text_position, language, font, '../templates/HdLogo.jpg')
                        st.image(ai_image)
                        cnt +=1
                    st.success(f"Selected images are enhanced with AI and saved to 'templates/aiImages' ")

            if st.button("Download Selected Images"):
                for i in images:
                    if i == 'main_thumbnail.jpg':
                        img = Image.open(os.path.join(output_path, i))
                    else:
                        img = Image.open(os.path.join(unique_thumbnails_folder, i))
            ...
            if st.button("Download All Images"):
                for thumbnail in selected_thumbnails:
                    if thumbnail == 'main_thumbnail.jpg':
                        img = Image.open(os.path.join(output_path, thumbnail))
                    else:
    ...
    # except Exception as e:
    #     print("Error:",str(e))
    #     st.warning("Something went wrong....Please Clear the session and try again.")
    e = time.time()
    print("Time:", (e-s))    
